...
voice = source[source.index("config APOLLO_CODEX_VOICE"):source.index("config APOLLO_NTP_SERVER")]
...
with tempfile.TemporaryDirectory() as directory:
    ...
    path.write_text("\n".join(
        f"config {name}\n    bool\n    default y\n"
        for name in ("APOLLO_PROTOCOL", "IDF_TARGET_ESP32S3", "BOARD_TYPE_WAVESHARE_ESP32_S3_TOUCH_LCD_1_85C")
    ) + voice + display)
    config = kconfiglib.Kconfig(str(path), warn=False)
    assert config.syms["APOLLO_CODEX_VOICE"].str_value == "y"
    ...
    config.syms["APOLLO_CODEX_VOICE"].set_value("n")
